...
const { createStatusEmailHTML, createStatusEmailText } = require("../utils/utils");
const fs = require("fs");
...
function getAllOrders(req, res) {
  ...
  Order
  .paginate({}, {page: req.query.page, limit: limit })
  .then(results => {
    res.json({ items: results.docs, pages: results.pages })
  })
  .catch(err => {
...
function update(req, res) {
  ...
  Order
    .findByIdAndUpdate(req.params.id, order)
    .then(() => res.json(true))
    .catch(console.log);
}
...
function addHistory(req, res) {
  ...
  Order
    .findByIdAndUpdate(orderID, { $push: { history: historyItem }, status: historyItem.status }, { new: true })
    .then(updatedOrder => {
  ...
  readFile("./settings/settings.json", "utf8")
    ...
    .then(settings => {
      ...
      const mailOptions = {
        ...
        text: createStatusEmailText(status, companyName, customerName, orderNumber),
        html: createStatusEmailHTML(status, companyName, customerName, orderNumber)
      };

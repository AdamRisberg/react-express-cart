...
class OrderAnalytics extends Component {
  ...
  fetchData() {
    ...
    api
      .get("/api/orders/analytics?by=" + this.state.graphRange.toLowerCase(), { cancelToken: this.cancelGetRequest.token }, true, true)
      .then(res => {
  ...
  createYearData() {
    ...
    return new Array(12).fill(0).map((_, i) => {
      if(this.state.data[counter] && this.state.data[counter]._id.month === i) {
  ...
  createMonthData() {
    ...
    return days.map(d => {
      if(this.state.data[counter] && this.state.data[counter]._id.dayOfMonth === d) {
        const count = this.state.data[counter].count;
        const totalPrice = this.state.data[counter].totalPrice;
        counter++;
  ...
  createWeekData() {
    ...
    return weekLabels.map((d,i) => {
      if(this.state.data[counter] && this.state.data[counter]._id.dayOfWeek === i+1) {
        const count = this.state.data[counter].count;
        const totalPrice = this.state.data[counter].totalPrice;
        counter++;
      ...
      <React.Fragment>
        ...
        <div className={styles.AnalyticsBox}>
          <label className={styles.ChartSelectLabel}>
            Show:
            <select className={styles.RangeSelect} onChange={this.handleChartContentChange} value={this.state.chartContent}>
          ...
          <label className={styles.ChartSelectLabel}>
            Range:
            <select className={styles.RangeSelect} onChange={this.handleRangeChange} value={this.state.graphRange}>
              <option>Week</option>
              <option>Month</option>
              <option>Year</option>
            </select>

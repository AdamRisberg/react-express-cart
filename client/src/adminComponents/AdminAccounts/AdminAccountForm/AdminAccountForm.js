...
class AdminAccountForm extends Component {
  ...
  fetchAdminAccount(id) {
    ...
    api
      .get(`/api/admin/${id}`, { cancelToken: this.cancelGetRequest.token }, true, true)
      .then(res => {
  ...
  handleFormSubmit = e => {
    ...
    if(!this.state.admin.hasPassword && this.state.admin.active && !this.validatePassword()) {
      return;
  ...
  saveChanges() {
    ...
    api
      .put("/api/admin/" + this.state.admin._id, {
        ...this.state.admin,
        password: this.state.newPassword
      }, { cancelToken: this.cancelPutRequest.token }, true, true)
      .then(res => {
  ...
  saveNewAdmin() {
    ...
    api
      .post("/api/admin", {
        ...this.state.admin,
        password: this.state.newPassword
      }, { cancelToken: this.cancelPostRequest.token }, true, true)
      .then(res => {
        this.props.history.push("/admin/admin_accounts");
      })
      .catch(err => {

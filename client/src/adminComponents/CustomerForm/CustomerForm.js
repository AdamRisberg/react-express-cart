...
import Address from "../../frontEndComponents/Address/Address";

import AddressForm from "../../frontEndComponents/Addresses/AddressForm/AddressForm";
...
class CustomerForm extends Component {
  ...
  componentDidMount() {
    ...
    api
      .get("/api/customers/" + customerID, { cancelToken: this.cancelGetRequest.token }, true, true)
      .then(res => {
  ...
  handleEditConfirm = (editedAddress) => {
    const editedAddressID = this.state.customer.addresses[this.state.editIdx]._id;
    ...
    return (
      <React.Fragment>
        <form onSubmit={this.handleFormSubmit} className={styles.CustomerForm}>
          ...
          <label>
            First Name
            <input onBlur={this.trimInputValue("firstName")} onChange={this.handleChange("firstName")} value={this.state.customer.firstName} />
          </label>
          <label>
            Last Name
            <input onBlur={this.trimInputValue("lastName")} onChange={this.handleChange("lastName")} value={this.state.customer.lastName} />
          </label>
          <label>
            Email
            <input onBlur={this.trimInputValue("email")} onChange={this.handleChange("email")} value={this.state.customer.email} />
          </label>
          <div className={`${styles.Addresses} ${styles.ClearFix}`}>
            {this.renderAddresses()}
          </div>
          {this.state.validateMessage ? (
            <div className={styles.ValidateMessage}>{this.state.validateMessage}</div>
          ) : null}

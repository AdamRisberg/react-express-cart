...
class PageForm extends Component {
  ...
  fetchPage(id) {
    ...
    api.get("/api/pages/" + id, { cancelToken: this.cancelGetRequest.token }, true, true)
      .then(res => {
  ...
  handleFormSubmit = (e) => {
    ...
    api.put("/api/pages/" + this.state.page._id, { page: this.state.page},  { cancelToken: this.cancelPutRequest.token }, true, true)
      .then(res => {
  ...
  render() {
    ...
    return (
      <form onSubmit={this.handleFormSubmit} className={styles.PageForm}>
        ...
        <label>
          Title
          <input onChange={this.handleChange("title")} value={this.state.page.title} />
        </label>
        <label>
          Content
          <TextEditor id="editor" style={{ fontWeight: "400" }} onChange={this.handleChange("content")} text={this.state.page.content} />
        </label>
        <label>
          Path
          <input onChange={this.handleChange("path")} value={this.state.page.path} />
        </label>
        <label>
          Meta Description
          <input onChange={this.handleChange("metaDescription")} value={this.state.page.metaDescription} />
        </label>

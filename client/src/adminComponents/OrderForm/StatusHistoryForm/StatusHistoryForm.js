...
const StatusHistoryForm = ({ status, notified, tracking, comment, onSubmit, onChange }) => {
  return (
    <React.Fragment>
      ...
      <div className={styles.ContentBox}>
        <form onSubmit={onSubmit}>
          ...
          <label>
            Tracking Number(s)
            <input onChange={onChange("tracking")} value={tracking} placeholder="Comma separated list"/>
          </label>
          <label>
            Comments
            <textarea onChange={onChange("comment")} value={comment} rows={5} />
          </label>
          <Button style={{ padding: "6px 30px" }} type="submit">ADD</Button>
        </form>
      </div>
    </React.Fragment>
  );

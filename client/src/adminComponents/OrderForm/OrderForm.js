...
import OrderView from "../../frontEndComponents/Order/OrderView/OrderView";
...
class OrderForm extends Component {
  ...
  componentDidMount() {
    ...
    api
      .get("/api/orders/" + this.props.match.params.id, { cancelToken: this.cancelGetRequest.token }, true, true)
      .then(res => {
  ...
  handleAddHistory = (e) => {
    ...
    api
      .post("/api/orders/history/" + this.state.order._id, historyItem, { cancelToken: this.cancelPostRequest.token }, true, true)
      .then(res => {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        <StatusHistoryForm {...this.state.history} onSubmit={this.handleAddHistory} onChange={this.handleHistoryChange} />
      </React.Fragment>
    );
  }
}

...
class CategoryForm extends Component {
  ...
  fetchCategory(id) {
    ...
    api
      .get("/api/categories/" + id, { cancelToken: this.cancelGetRequest.token }, false, false)
      .then(res => {
  ...
  fetchAllCategories() {
    ...
    api
      .get("/api/categories", { cancelToken: this.cancelGetAllRequest.token }, false, false)
      .then(res => {
  ...
  handleDeleteImage = (image) => () => {
    ...
    api
      .delete(
        `/api/categories/${categoryID}/image?location=categories&file=${image.src}`,
  ...
  handleParentChange = (e) => {
    ...
    const path = parentPath ? parentPath + "/" + this.state.category.pathName : this.state.category.pathName;
  ...
  handleImageSelect = (e) => {
    ...
    api
      .put("/api/categories/" + categoryID + "/image?location=categories", data, {
  ...
  render() {
    ...
    const image = this.state.category.image ? [{ src: this.state.category.image, alt: "" }] : []; 
    ...
    return (
      <form onSubmit={this.handleFormSubmit} className={styles.CategoryForm}>
        ...
        <label>
          Name
          <input onChange={this.handleNameChange} value={this.state.category.name} />
        </label>
        <label>
          Path Name
          <input onChange={this.handlePathNameChange} value={this.state.category.pathName} />
        </label>
        <label>
          Path
        ...
        <label>
          Parent
          <CategorySelect categories={this.state.categories} onChange={this.handleParentChange} value={this.state.category.parent} />
        </label>
        <label>
          Meta Description
          <input onChange={this.handleTextChange("metaDescription")} value={this.state.category.metaDescription} />
        </label>
        <label>
          Order
          <input onChange={this.handleTextChange("order")} defaultValue={this.state.category.order === "a" ? "" : this.state.category.order} />
        </label>

...
class Customers extends Component {
  ...
  handleDelete = () => {
    ...
    api
      .post("/api/customers/delete", Object.keys(this.state.selected), { cancelToken: this.cancelDeleteRequest.token }, true, true)
      .then(res => {
          ...
          <table className={styles.Table}>
            ...
            <tbody>
              {customers.map(customer => (
                <tr key={customer._id}>
                  <td><Checkbox handleSelect={this.handleSelect} isChecked={this.state.selected[customer._id]} id={customer._id} /></td>
                  <td>{customer.firstName} {customer.lastName}</td>
                  <td className={styles.CollapseLarge}>{customer.email}</td>
                  <td className={styles.CollapseSmall}>{formatDateShort(customer.dateAdded)}</td>
                  <td>
                    <Button type="edit" onClick={this.handleEdit(customer._id)} />
                  </td>
                </tr>
              ))}
            </tbody>

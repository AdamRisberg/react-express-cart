...
import api from "../../../api";
import { handleAdminRequestError, handleAdminRequestErrorFull} from "../../../utils";
...
class ShippingForm extends Component {
  ...
  fetchShipping(id) {
    ...
    api
      .get(`/api/shipping/${id}`, { cancelToken: this.cancelGetShippingRequest.token }, true, true)
      .then(res => {
  ...
  handleFormSubmit = (e) => {
    ...
    api
      .put("/api/shipping/" + this.state.shipping._id, this.state.shipping, { cancelToken: this.cancelPutShippingRequest.token }, true, true)
      .then(res => {
    ...
    return (
      <React.Fragment>
        ...
        <form onSubmit={this.handleFormSubmit} className={styles.Method}>
          <label>
            Name
            <input type="text" onChange={this.onChange("name")} value={this.state.shipping.name} />
          </label>
          <label>
            Label
            <input type="text" onChange={this.onChange("label")} value={this.state.shipping.label} />
          </label>
          <label>
            Price
            <input type="number" onChange={this.onChange("price")} value={this.state.shipping.price} />
          </label>
          <label>
            Active
            <select onChange={this.onChange("active")} value={this.state.shipping.active}>
              <option value={true}>Yes</option>
              <option value={false}>No</option>
            </select>
          </label>
          <Button onClick={this.handleCancel} type="cancel" float="left">Cancel</Button>

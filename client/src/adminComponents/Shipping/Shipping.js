...
class Shipping extends Component {
  ...
  handleDelete = () => {
    ...
    api
      .post("/api/shipping/delete", Object.keys(this.state.selected), { cancelToken: this.cancelDeleteRequest.token }, true, true)
      .then(res => {
  ...
  handleAdd = () => {
    ...
    api
      .post("/api/shipping/", {}, { cancelToken: this.cancelPostRequest.token }, true, true)
      .then(res => {
          ...
          <table className={styles.Table}>
            ...
            <tbody>
              {shipping.map(method => {
                return (
                  <tr key={method._id}>
                    <td><Checkbox handleSelect={this.handleSelect} isChecked={this.state.selected[method._id]} id={method._id} /></td>
                    <td>{method.name}</td>
                    <td className={styles.CollapseLarge}>{method.label}</td>
                    <td className={styles.CollapseSmall}>${method.price.toFixed(2)}</td>
                    <td className={styles.Collapse}>{method.active ? "Yes" : "No"}</td>
                    <td>
                      <Button type="edit" onClick={this.handleEdit(method._id)} />
                    </td>
                  </tr>
                );
              })}

...
class Products extends Component {
  ...
  handleDelete = () => {
    ...
    api
      .post("/api/products/delete", Object.keys(this.state.selected), { cancelToken: this.cancelDeleteRequest.token }, true, true)
      .then(res => {
  ...
  handleAdd = () => {
    ...
    api
      .post("/api/products/", { info: [{ title: "Description", body: "" }] }, { cancelToken: this.cancelPostRequest.token }, true, true)
      .then(res => {
          ...
          <table className={styles.Table}>
            ...
            <tbody>
              {products.map(product => {
                const image = { src: "/images/no-image.jpg", alt: "No preview" };
                ...
                return (
                  <tr key={product._id}>
                    <td><Checkbox handleSelect={this.handleSelect} isChecked={this.state.selected[product._id]} id={product._id} /></td>
                    <td className={styles.CollapseSmall}>
                      <img src={image.src} alt={image.alt} />
                    </td>
                    <td>{product.name}</td>
                    <td className={styles.Collapse}>{product.model}</td>
                    <td className={styles.Collapse}>${product.price.toFixed(2)}</td>
                    <td>
                      <Button type="edit" onClick={this.handleEdit(product._id)} />
                    </td>
                  </tr>
                );
              })}

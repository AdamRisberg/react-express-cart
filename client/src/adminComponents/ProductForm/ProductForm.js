...
import api from "../../api";
import { handleAdminRequestErrorFull, handleAdminRequestError } from "../../utils";
import { Redirect } from "react-router-dom";

import Spinner from "../../frontEndComponents/Spinner/Spinner";
...
class ProductForm extends Component {
  ...
  fetchProduct(id) {
    ...
    api
      .get(`/api/products/${id}`, { cancelToken: this.cancelGetProductReqeust.token }, true, true)
      .then(res => {
  ...
  fetchCategories() {
    ...
    api.get("/api/categories", { cancelToken: this.cancelGetCatsReqeust.token }, false, true)
      .then(res => {
  ...
  handleImageSelect = (e) => {
    ...
    data.append("file", file, `${productID}-${this.state.product.images.length}-${file.name}`);

    api
      .put("/api/products/" + productID + "/image?location=products", data, {
  ...
  handleDeleteImage = (image) => () => {
    ...
    api.delete(`/api/products/${productID}/image?location=products&file=${image.src}`, { cancelToken: this.cancelDeleteReqeust.token }, true, true)
      .then(res => {
        this.setState(() => ({ 
          product: {
            ...this.state.product,
  ...
  saveChanges() {
    ...
    api
      .put("/api/products/" + this.state.product._id, this.state.product, { cancelToken: this.cancelPutProductReqeust.token }, true, true)
      .then(res => {
  ...
  saveNewProduct() {
    ...
    api
      .post("/api/products", this.state.product, { cancelToken: this.cancelPostProductReqeust.token }, true, true)
      .then(res => {
        this.props.history.push("/admin/products");
      })
      .catch(handleAdminRequestErrorFull(this.props.flashErrorMessage));
  ...
  renderGeneral() {
    return (
      <React.Fragment>
        ...
        <label>
          Name
          <input onChange={this.handleInputChange("name")} value={this.state.product.name} />
        </label>
        <label>
          Model
          <input onChange={this.handleInputChange("model")} value={this.state.product.model} />
        </label>
        <label>
          Price
          <input onChange={this.handleInputChange("price")} type="number" value={this.state.product.price} />
        </label>
        <label>
          Featured
          <select onChange={this.handleInputChange("featured")} value={this.state.product.featured}>
        ...
        <label>
          Meta Description
          <textarea onChange={this.handleInputChange("metaDescription")} value={this.state.product.metaDescription} />
        </label>
        <Images
    ...
    return (
      <React.Fragment>
        ...
        {this.state.product.info.map((info, i) => (
          <React.Fragment key={info._id + "" + i}>
            <div className={`${styles.SubHeader} ${styles.ClearFix}`}>
              ...
              {i !== 0 ? (
                <Button style={{ fontSize: ".9rem", padding: "4px 8px", boxShadow: "none" }} type="delete" onClick={this.handleDeleteInfo(i)} />
              ) : null}
            </div>
            <div className={styles.InfoTab}>
              <label>
                Title
                <input value={info.title} onChange={this.handleInfoTitleChange(i)} />
              </label>
        ...
        <div className={styles.Black}>
          <button type="button" className={styles.AddTab} onClick={this.handleAddInfo}>Add Info Tab</button>
        </div>
      </React.Fragment>
    );
  }
    ...
    return (
      <React.Fragment>
        ...
        <div className={styles.AddOptionButtons}>
          <button type="button" onClick={this.handleAddOption("text")}>Text</button>
          <button type="button" onClick={this.handleAddOption("select")}>Select</button>
          <button type="button" onClick={this.handleAddOption("textRadio")}>Text Radio</button>
          <button type="button" onClick={this.handleAddOption("colorRadio")}>Color Radio</button>
        </div>
        {this.state.product.options.map((option, optionIdx) => (
          ...
          <div className={`${styles.SubHeader} ${styles.ClearFix}`}>
            ...
            <Button style={{ fontSize: ".9rem", padding: "4px 8px", boxShadow: "none" }} type="delete" onClick={this.handleDeleteOption(optionIdx)} />
          </div>
          <div className={styles.OptionBox}>
            <label className={styles.Underline}>
              Option Name
              <input onChange={this.handleOptionNameChange(optionIdx)} value={option.label} />
            </label>
            {option.options.map((optionValue, optionValueIdx) => this.renderOptionValue(option, optionIdx, optionValue, optionValueIdx))}
            {option.optionType !== "text" ? (
              <button type="button" className={styles.AddTab} onClick={this.handleAddOptionValue(option.optionType, optionIdx)}>Add Option Value</button>
            ) : null}
          </div>
          </React.Fragment>
        ))}
  ...
  handleOptionValueChange = (key, optionIdx, optionValueIdx) => (e) => {
    ...
    if(key === "price") {
      ...
      if(isNaN(value)) {
        value = this.state.product.options[optionIdx].options[optionValueIdx].price;
      }
    ...
    optionValues[optionValueIdx] = { ...optionValues[optionValueIdx], [key]: value };
    options[optionIdx].options = optionValues;
  ...
  openCloseColorPicker = (optionIdx, optionValueIdx, open) => () => {
    const options = this.state.product.options.slice();
    const optionValues = options[optionIdx].options.slice();
    optionValues[optionValueIdx] = { ...optionValues[optionValueIdx], showPicker: open };
    options[optionIdx].options = optionValues;
  ...
  renderOptionValue(option, optionIdx, optionValue, optionValueIdx) {
    return (
      <div key={optionValue._id + "" + optionValueIdx} className={styles.Underline}>
        <label>
          Option Value Name
          <input onChange={this.handleOptionValueChange("label", optionIdx, optionValueIdx)} value={optionValue.label} />
        </label>
        {option.optionType === "colorRadio" ? (
          <div className={styles.ColorLabel} aria-label="Option Value Color">
            ...
            {optionValue.showPicker ? (
              <OptionColorPicker
                close={this.openCloseColorPicker(optionIdx, optionValueIdx, false)}
                onChange={this.handleOptionValueChange("color", optionIdx, optionValueIdx)}
              ...
              <button
                ...
                onClick={this.openCloseColorPicker(optionIdx, optionValueIdx, true)}
              >
                <div style={{ backgroundColor: optionValue.color }} className={styles.ColorPreview} />
              </button>
            )}
          </div>
        ) : null}
        {option.optionType !== "text" ? (
          <label>
            ...
            <input onBlur={this.handleOptionValueChange("price", optionIdx, optionValueIdx)} type="number" defaultValue={optionValue.price.toFixed(2)} />
          </label>
        ) : null}
        <div className={styles.ClearFix}>
          <Button
            style={{ fontSize: ".9rem", padding: "4px 8px", boxShadow: "none", marginBottom: "20px" }}
            type="cancel"
            onClick={this.handleDeleteOptionValue(optionIdx, optionValueIdx)}
          >Delete Option Value</Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <React.Fragment>
        <form className={styles.ProductForm} onSubmit={this.handleFormSubmit}>
          ...
          <div className={styles.ClearFix}>
            <Button type="cancel" float="left" onClick={this.handleCancel}>Cancel</Button>
            <Button type="submit" float="right" onClick={this.handleFormSubmit}>SAVE</Button>
          </div>
        </form>

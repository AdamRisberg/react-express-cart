...
const NavItem = (props) => {
  ...
  return (
    <li className={hasSubs ? styles.NavItemWithSubs : styles.NavItem}>
      ...
      {hasSubs ? renderSubcategories(props.subcategories, props.right, props.prefix) : null}
    </li>
  );
};
        ...
        {cats.map(cat => {
          ...
          return (
            <React.Fragment key={cat._id}>
              <div className={hasSubs ? styles.SubItemWithSubs : styles.SubItem}>
                {cat.clickHandler ? (
                  <span onClick={cat.clickHandler} className={styles.SubItemLink}>{cat.name}</span>
                ) : (
                  <Link to={`${prefix}/${cat.path}`} className={styles.SubItemLink} >
                      {cat.name}
                  </Link>
                )}
                {hasSubs ? renderSecondLevel(cat.subcategories, right, prefix) : null}
              </div>
            </React.Fragment>
          );
        })}
...
function renderSecondLevel(cats, right, prefix) {
  return (
    <React.Fragment>
      ...
      <div className={right ? styles.SecondLevelBoxRight : styles.SecondLevelBox}>
        {cats.map(cat => {
          return (

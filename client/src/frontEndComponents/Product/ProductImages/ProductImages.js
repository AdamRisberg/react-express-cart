...
class ProductImages extends Component {
  ...
  handleImageSelect = (index) => {
    ...
    this.setState({ selectedIdx: index, selectedImg: this.props.images[index] });
  }
  ...
  renderImages() {
    return this.props.images.map((img, i) => {
      return <ImageLoader
        className={i === this.state.selectedIdx ? `${styles.PreviewImage} ${styles.SelectedPreview}` : styles.PreviewImage}
        key={i}
        imageName={img.src}

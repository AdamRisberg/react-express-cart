  ...
  return (
    <React.Fragment>
      ...
      <select
        ...
        onChange={(e) => inputChange(_id, options[e.target.options.selectedIndex].price, e.target.value)}>
        {selectOptions}
      </select>
    </React.Fragment>
  );
...
function createTextInput({ _id, inputChange, value, label }) {
  return (
    <React.Fragment>
      <label htmlFor={label} className={styles.Label}>{label}</label>
      <input id={label} type="text" className={styles.Input} value={value} onChange={e => inputChange(_id, 0, e.target.value)} />
    </React.Fragment>
  );
}
        ...
        return (
          <label key={option.label + "l"} className={styles.ColorRadioLabel}>
            ...
            <span className={styles.ColorRadio}>
              <span style={{ backgroundColor: option.color }} className={styles.Color}></span>
            </span>
          </label>
        );
      })}

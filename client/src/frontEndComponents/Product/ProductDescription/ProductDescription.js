        ...
        <div className={`${styles.TitleRow} ${styles.Show}`}>
          {info.map((content, i) => {
            return (
              <div
                key={content.title}
                className={i === this.state.selectedIdx ? `${styles.TitleBox} ${styles.Selected}` : styles.TitleBox}
        ...
        {info.map((content, i) => {
          ...
          return (
            <React.Fragment key={content.title}>
              <h4 className={`${styles.Title} ${styles.Hidden}`}>{content.title}</h4>
              <RenderHTML className={`${styles.DescriptionBox} ${showStyle} ${lastStyle}`} html={content.body} />
            </React.Fragment>
          );
        })}
      </div>

...
class WithPagination extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if(this.state.fetchData || this.props.fetchUrl !== this.state.fetchUrl ||
      prevState.filter !== this.state.filter || prevProps.refresh !== this.props.refresh) {
  ...
  fetchData = (resetPage) => {
    ...
    this.setState({ fetchData: false, fetchUrl: this.props.fetchUrl, loading: true });
    ...
    api.get(url, { cancelToken: this.cancelTokenSource.token }, this.props.fetchUseSession, this.props.fetchAdmin)
      .then(results => {
        const stateUpdate = {
  ...
  render() {
    return (
      <div style={{ minHeight: "1px" }}>
        {this.props.renderTitle ? this.props.renderTitle(this.state.items.length) : null}
        {this.state.loading ? <Spinner /> : (
          <React.Fragment>
            {this.props.renderFilters ? this.props.renderFilters(this.state.filter, this.handleFilterChange) : null}
